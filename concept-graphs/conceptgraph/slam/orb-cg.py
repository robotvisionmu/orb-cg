...
from conceptgraph.custom.utils import *
import logging
import time
from conceptgraph.custom.ReRunWrapper import *
...
@hydra.main(version_base=None, config_path="../hydra_configs/", config_name="orb-cg")
def main(cfg : DictConfig):
    ...
    vocab_file = "../../orbslam_files/ORBvoc.txt"
    settings_file = "../../orbslam_files/replica_settings.yaml"
    # settings_file = "../../orbslam_files/realsense_settings.yaml"
    dataset_path = os.path.join(cfg.dataset_root, cfg.scene_id)
        ...
        for kf_id in new_keyframe_ids:
            ...
            if len(object_database) == 0:
                for det_idx in range(len(detections['masks'])):
                    ...
                    obj_id = count
                    count += 1
                    new_obj = BoomObject(obj_id, detections['class_labels'][det_idx], detections['pcds_world'][det_idx], detections['bboxes_3d'][det_idx], detections['clip_fts'][det_idx])
